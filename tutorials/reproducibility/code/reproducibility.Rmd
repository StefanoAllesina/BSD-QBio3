...
Avoid the need to write lots of explanatory commments by writing Really Obvious Code (ROC). Tips for writing ROC:
* use function and variable names that are self-explanatory. For example `random_locations_of_N_spiders_in_a_box.R` is a much better function name than `eek.R`, and `eek2017.R` is still better than `eek.R`. Old coders had to limit variable name and code lengths because of the limitations of memory size. You're no longer constrained in this way! 

## DIY lottery
To practice commenting code, let's write a short program to generate a draw from a lottery. The lottery is a somewhat peculiar one: The winning lottery number is drawn by flipping a coin 30 times and reporting a "$1$" for heads and a "$0$" for tails. The same bent coin is always used, and it has a probability of $0.1$ of turning up heads and a probability of $0.9$ of turning up tails. Each ticket for the lottery is a buyer-determined string of $30$ $1$'s and $0$'s, and costs $\$1$. The prize money for the lottery is $\$10,000,000$. 
...
Next, let's explore how we simulate flipping this bent coin once. Here's one way to do it:
```{r,eval=TRUE}
p_heads <- 0.1
random_val <- runif(1)
...
You will notice that the function `RNGkind` is the interface for querying the current state of the RNG. Let's find out what the current settings are:
...
The seed to an RNG is usually a large integer that provides an initialization the RNG algorithm. Scrolling down to "Note" you'll learn that the seed to `R`'s RNG is set by the current time and sometimes the process ID. That means that your simulation results will depend on when you start your `R` session, run your code, and even some local information in your processing environment. Compare the output of `runif` with your neighbor.
...
Besides these basic commands, `git` allows you to do much more. We
...
once you run this command, the file will be in the last
...
There are two main options: a) setup your own server (this is beyond the scope of this introduction, but see Section \ref{sec:gitrr} for pointers on how to do this); b) host your repositories on a website offering this service (for a fee, or for free).

The most popular option for hosting Open Source projects, where the whole world can see what and when you commit, is [GitHub](http://github.com), which will store all your public repositories for free. 
...
* Start making changes. When done run:
```{r, engine = 'bash', eval = FALSE}
$ git add --all
```
* Time to commit:
...
* Ok, everyone should see my changes
```{r, engine = 'bash', eval = FALSE}
$ git push
```
...
creates the new branch `fastercode`. 
...
which means that now you can experiment freely. Suppose that you edit the code, and commit the changes:

* Change the code, and then commit
```{r, engine = 'bash', eval = FALSE}
...
## Data Challenge
This tutorial has been focused on making your code reproducible and sharable with the wider world. Many journal now *require* that you submit your code and data when you publish your paper. One such article was recently written on the False Discovery Rate in medical research (Jaeger and Leek (2014) can be found in the `readings` folder). While an earlier article claimed that over 80% of medical results were probably false discoveries, this work uses a slightly more sophisticated analysis of the p-values reported in the medical literature, to determine whether the observed distribution of p-values is significantly different from those drawn from a uniform distribution. Reassuringly, the authors find that the false discovery rate is lower (~20%), but still troubling, across many medical journals. 

* 1) Use the published code in:
Jager, Leah R., and Jeffrey T. Leek. "An estimate of the science-wise false discovery rate and application to the top medical literature." Biostatistics 15.1 (2013): 1-12.
...to reproduce the results of the paper. In particular, report the mean and standard deviaiont of the FDR of the journal The Lancet to 4 significant digits.
...
* 3) Bonus question: use the authors' code to first scrape PubMed for abstracts, and next compute the FDR for the Journal of Psychiatric Neuroscience. You will find a bug in the code that you will need to hunt down and fix.
