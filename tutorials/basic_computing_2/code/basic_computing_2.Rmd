...
*I found that the structure of this tutorial did not work as well as
Basic Computing 1; the materials were too dense, and some
mini-exercises sprinkled throughout would be helpful for making the
lesson more interactive and less "lecture-like". Also, I found the
"payoff" of this tutorial unclear. What sort of insights are we
suppose to arrive at after doing all this work of implementing these
sophisticated statistical analyses? ---Peter*
...
or call the command into your script.
...
*Note:* Which package is `cor` from? Why didn't we need to load the
package? (Run `loadedNamespaces()` for the answer.)

To perform a significance test, use

```{r}
out <- cor.test(papers$TitleLength, papers$Cited.by, method = "kendall")
print(out)
```
...
```{r}
cor.test(rnorm(1000),rnorm(1000))
```
...
*Warning:* `lm` can do many, many things and can be called in many
different ways ("Swiss Army knife"); we will only show you one way of
using this function.
...
```{r}
model_cits <- lm(LogCits ~ TitleLength,papers)
...
*Instructor note:* I prefer the invocation `lm(formula,data.frame)`
because I find it easier to remember, and because the `lm(formula)`
invocation uses non-standard evaluation which can lead to errors that
are difficult to debug unless you understand R's model of function
argument evaluation (which is based on Lisp/Scheme, I believe). ---Peter
...
```{r}
papers <- transform(papers,Year = as.factor(Year))
model_year_length <- lm(LogCits ~ Year + TitleLength,papers)
...
*Instructor note:* Functions are one of the most important concepts in
programming. Functions bring to your program FOUR desirable aspects:
(1) block structure, (2) modularity, (3) abstraction, and (4)
encapsulation. (Note 3 and 4 are different features thought commonly
confused with each other; you can have abstraction without
...
*I used a less trivial example in class that I think worked well and
promoted discussion: computing the roots of a quadratic function using
the quadratic formula. It is a simple function---at first---but then
one needs to consider all sorts of special cases (e.g., when the
discrimant is equal to zero, or negative, or when a is zero). So this
also is a good opportunity to work in if-else statements and multiple
return statements.*
...
*Maybe make this function more generic? Why not call the arguments x
and y instead of citations and titlelength? There is nothing about the
code that is particular to citations and title lengths.*
...
*It would be good to integrate this section a little better with the
rest of the lesson. It would be cool use some of our previous code to
create a small R Markdown document and render it. Alternatively, we
should have a couple prepared examples illustrating what you can do
...
> Computers are good at consuming, producing and processing data. Humans,
> on the other hand, process the world through narratives. Thus, in order
> for data, and the computations that process and visualize that data, to
> be useful for humans, they must be embedded into a narrative---a
> computational narrative---that tells a story for a particular audience
> and context.
>
> [Fernando Perez, 2015](https://blog.jupyter.org/2015/07/07/project-jupyter-computational-narratives-as-the-engine-of-collaborative-data-science)
...
3. Write a function to do 1 and 2.
4. The data spans 2010-2013. In Aug 2013 Google Flu changed their algorithm. Did this lead to improvements? Compare the data from Aug to Dec 2013 with the same months in 2010, 2011, and 2012. For each, calculate the correlation, and see whether the correlation is higher for 2013.

**Hints** You will need to extract the year from a string for each row. To do so, you can use `substr(google$WeekCommencing, 1,4)` or `strsplit`.
