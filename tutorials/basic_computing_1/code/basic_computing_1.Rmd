...
*Instructor note:* One difficulty with presenting this material is
that a period and comma look very similar on the screen. Maybe a
judicious voice of font for typing the code (e.g., Verdana, which has
a more distinguishable comma and period) would help with this.
...
*Add brief section here explaining WHY we are learning R, and not some
other programming language, e.g., C++, Java, Python.*
...
*Instructor note:* I think it is also valuable to briefly show what R
looks like when run from the command-line shell, e.g., Terminal on Mac.
...
A few setup details to make sure that everyone is good to go:

1. Set your working directory.

2. Create a new .R text file (e.g., `basic_computing1.R`, and document
all the commands you write in this file.

3. *Anything else?*

# The most basic operation: assignment

*Instructor note:* This section and the next few sections are a little
on the boring side, but you need to learn the basics before you can do
the fun stuff (e.g., analyzing data, creating cool plots).
...
*Instructor note:* This is a great opportunity to break down this statement carefully and explain what is going on.
...
- `numeric`, storing real numbers (actually, their approximations, as computers have limited memory and thus cannot store numbers like $\pi$, or even $1/3$)
```{r}
v <- 3.77
class(v)
```

We can also create numbers using scientific notation, e.g., Avogadro's
constant:
...
*Instructor note:* One basic data type that is missing from this list
is a "factor". However, it is difficult to see why factors are useful
until you have a data frame, so we will introduce them when we explore
data frames below.
...
*Instructor note:* I didn't have time to cover all these functions,
...
errors. (Unlike languages such as C and Java, R is not "strongly
typed.") If sensible, you can convert a type into another. For
example:
...
*Need to introduce parentheses here. This is a good simple example:
does `-2^2` give you the square of negative two? Why or why not?*
...
*Why are two equal signs being used here?*
...
*Add an example here with the "not" logical operation.*
...
The most basic data structure in `R` is the vector, which is an ordered collection of values of the same type. Vectors can be created by concatenating different values with the "combine" function `c()`:
...
`NA` stands for Not Available ("missing data"). Other special values are `NaN` (Not a Number, e.g., `0/0`), `Inf` (Infinity, e.g., `1/0`), and `NULL` (variable not set). You can test for special values using `is.na(x)`, `is.infinite(x)`, etc.

*Important:* Note that in `R` a single number (string, logical) is a vector of length 1 by default. That's why if you type `3` in the console you see `[1] 3` in the output.
...
*Instructor note:* Explain what is `1:3`. It is also a vector!
...
The summary function is one of the most important functions in R
because it works on many types of variables.
...
*Instructor note:* There are some other useful vriants of `seq` and `rep`.
...
*Instructor note:* This section may be skipped if you are short on time.
...
So far, everything we have seen so far was either a matrix or a
vector. *We are now leaving "vector world."*

*Instructor note:* I would consider a list to be an advanced data
structure that will rarely be created by a beginning programmer. But
it is useful to point out that the return values of functions are
often lists (e.g., `lm`).
...
*Instructor note:* At this point, you should check in with the
participants and have them run `getwd()` and, if necessary,
`setwd(...)`, so that everyone is in the right working directory for
loading the HGDP data set.
...
*Instructor note:* This is our first encounter with a complicated
function---`read.csv` has over 20 arguments! Fortunately we don't need
to know what all these arguments do because the `defaults` are usually
sensible (but not always; e.g., `stringsAsFactors` which we will
encounter in Basic Computing 2).
...
*Exercise:* Change `x <- 4` to `x <- round(rnorm(1))`.
...
You will work with your own group to solve the following exercise. When you have found the solution, go to `https://stefanoallesina.github.io/BSD-QBio3/` and follow the link `Submit solution to challenge 1` to submit your answer (alternatively, you can go directly to [goo.gl/forms/dDJKvF0dOi7KUDqp1](https://goo.gl/forms/dDJKvF0dOi7KUDqp1). At the end of the boot camp, the group with the highest number of correct solutions will be declared the winner. If you need extra time, you can work with your group during free time or in the breaks in the schedule.
...
- Save your solution code for ech exercise in an .R file.
